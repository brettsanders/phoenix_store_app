defmodule Storex.SalesTest do
  ...
  describe "sales_carts" do
    ...
    test "list_sales_carts/0 returns all sales_carts" do
      cart = cart_fixture()
      assert Sales.list_sales_carts() == [cart]
    end
    ...
    test "update_cart/2 with valid data updates the cart" do
      cart = cart_fixture()
      assert {:ok, cart} = Sales.update_cart(cart, @update_attrs)
      assert %Cart{} = cart
    end
  ...
  describe "sales_line_items" do
    ...
    test "update_line_item/2 with valid data updates the line_item" do
      line_item = line_item_fixture()
      assert {:ok, line_item} = Sales.update_line_item(line_item, @update_attrs)
      assert %LineItem{} = line_item
